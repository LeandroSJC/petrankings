...
const PROTEINAS_ANIMAIS_REGEX =
  /(farinha\s+de\s+(v[ií]sceras|carne|frango|aves|peixe|salm[aã]o|cordeiro|peru|su[ií]no))|(carne\s+mecanicamente\s+separada)|(carne\s+(desidratada|fresca|bovina|de\s+frango|de\s+salm[aã]o|de\s+su[ií]no|de\s+cordeiro|de\s+peru|de\s+atum))|(peito\s+de\s+(frango|peru|aves))|(fil[eé]\s+de\s+(frango|salm[aã]o|peixe|atum))|((v[ií]sceras|mi[uú]dos|f[ií]gado|cora[cç][aã]o)\s+de\s+(aves|frango|bovino|su[ií]no|peru|peixe))|(ovo\s+em\s+p[oó])|(prote[ií]na\s+isolada\s+de\s+(su[ií]no|frango))|(\b(salm[aã]o|atum|sardinha)\b)/i;
